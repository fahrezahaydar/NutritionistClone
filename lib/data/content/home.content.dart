String heroTitle = 'Personalized Nutrition Coaching';
String heroSubtitle = 'Transform Your ❤️ Health with';
String heroBody =
    'Welcome to Nutritionist, your partner in achieving optimal health through personalized nutrition coaching. Our certified nutritionists are here to guide you on your weight loss journey, providing customized plans and ongoing support. Start your transformation today and experience the power of personalized nutrition coaching.';

List<Map<String, String>> featuresData = [
  {
    "icon": 'camera.macro',
    "title": "Personalized Nutrition Plans",
    "subtitle":
        "Receive a tailored nutrition plan designed specifically for your body and goals. Our certified nutritionists will consider your unique needs, dietary preferences, and health conditions to create a plan that suits you best."
  },
  {
    "icon": 'fork.knife',
    "title": "Food Tracking and Analysis",
    "subtitle":
        'Effortlessly track your food intake using our user-friendly app. Our nutritionists will analyze your data to provide insights into your eating habits, help you identify areas for improvement, and make personalized recommendations.'
  },
  {
    "icon": 'dumbbell.fill',
    "title": "Lifestyle and Behavior Coaching",
    "subtitle":
        "Achieving sustainable results requires more than just a diet plan. Our nutritionists will work with you to develop healthy habits, address emotional eating, and provide strategies to overcome obstacles along the way."
  },
  {
    "icon": 'graduationcap.fill',
    "title": "Guidance from Certified Nutritionists",
    "subtitle":
        "Our team of experienced and certified nutritionists will provide professional guidance and support throughout your journey. They will answer your questions, address your concerns, and keep you motivated as you work towards your goals."
  },
  {
    "icon": 'wallet.pass.fill',
    "title": "Meal Planning and Recipes",
    "subtitle":
        "Access a vast collection of delicious and healthy recipes tailored to your dietary needs. Our nutritionists will also create personalized meal plans, making it easier for you to stay on track and enjoy nutritious meals."
  },
  {
    "icon": 'backpack.fill',
    "title": "Nutritional Education and Workshops",
    "subtitle":
        "Expand your knowledge of nutrition through informative articles and educational workshops. Our nutritionists will equip you with the knowledge and tools to make informed choices for long-term success."
  }
];

Map<String, String> featuresHeader = {
  "title": 'Features',
  "subtitle":
      'Welcome to the Feature Section of Nutritionist, your ultimate destination for all things nutrition and wellness.',
};

Map<String, String> blogHeader = {
  "title": 'Our Blogs',
  "subtitle":
      "Our blog is a treasure trove of informative and engaging articles written by our team of nutritionists, dietitians, and wellness experts. Here's what you can expect from our blog."
};

var blogData = [
  {
    "subtitle": "Weight Loss",
    "title": "Staying Hydrated for Weight Loss and Overall Health",
    "description":
        "Discover how staying hydrated can support your weight loss goals and improve overall health",
    "image": "blog1",
  },
  {
    "subtitle": "Cultivating a Healthy Relationship with Food",
    "title": "Mindful Eating for a Healthier You",
    "description":
        "Learn how practicing mindful eating can help you develop a healthier relationship with food and improve your overall well-being",
    "image": "blog3"
  },
  {
    "subtitle": "Understanding Macronutrients",
    "title": "Carbohydrates, Proteins, and Fats",
    "description":
        "Get a comprehensive understanding of macronutrients and their role in your diet for optimal health and weight management.",
    "image": "blog2"
  },
  {
    "subtitle": "Healthy Snacks on the Go",
    "title": "Quick and Nutritious Options",
    "description":
        "Explore a variety of convenient and healthy snack ideas to keep you fueled throughout the day.",
    "image": "blog4"
  }
];

Map<String, String> testimonialHeader = {
  "title": 'Our Testimonials',
  "subtitle":
      'Our satisfied clients share their success stories and experiences on their journey to better health and well-being.',
};

List<Map<String, String>> testimonialData = [
  {
    "name": "John Doe",
    "testimonial":
        "I've been working with the nutritionist for three months, and the results are amazing! The personalized meal plans and guidance have made a significant difference in my health and energy levels."
  },
  {
    "name": "Alice Smith",
    "testimonial":
        "The nutrition coaching provided by Nutritionist has been a game-changer for me. I appreciate the attention to detail and the tailored approach to my dietary needs. Highly recommended!"
  },
  {
    "name": "David Johnson",
    "testimonial":
        "I've struggled with my weight for years, but with the support of Nutritionist, I've seen sustainable changes. The coaching is not just about diets; it's about building a healthier lifestyle. Thank you!"
  },
  {
    "name": "Emily Brown",
    "testimonial":
        "Nutritionist has helped me understand the importance of balanced nutrition. The guidance is practical, and I feel more confident in making informed food choices. Grateful for the positive impact on my well-being."
  },
  {
    "name": "Sophie Anderson",
    "testimonial":
        "Working with Nutritionist has been a transformative experience. The coaching goes beyond nutrition; it's a holistic approach to wellness. I've never felt better!"
  },
  {
    "name": "Michael Chen",
    "testimonial":
        "Nutritionist's expertise has guided me towards healthier food choices without sacrificing flavor. The sustainable changes are making a positive impact on my fitness goals."
  },
  {
    "name": "Olivia Rodriguez",
    "testimonial":
        "I appreciate the individualized attention from Nutritionist. The meal plans are realistic and easy to follow, making the journey to better health enjoyable."
  },
  {
    "name": "Daniel Taylor",
    "testimonial":
        "Nutritionist's support has been instrumental in my weight loss journey. The coaching is motivational, and I've learned to make healthier choices that fit my lifestyle."
  },
  {
    "name": "Grace Turner",
    "testimonial":
        "The nutrition coaching is tailored to my preferences and dietary restrictions. It's not just about restriction; it's about finding joy in nourishing my body. Highly recommended!"
  },
  {
    "name": "Liam Parker",
    "testimonial":
        "I was skeptical about nutrition coaching, but Nutritionist proved me wrong. The guidance is practical, and the results speak for themselves. Feeling healthier and more energized!"
  },
  {
    "name": "Ava Nguyen",
    "testimonial":
        "Nutritionist has helped me break unhealthy eating habits. The personalized approach and ongoing support make it easy to stay on track. Thank you for the positive change!"
  },
  {
    "name": "Ethan Miller",
    "testimonial":
        "I've tried various diets before, but Nutritionist's coaching is different. It's about sustainable lifestyle changes. The results are not just physical; they're mental and emotional too."
  },
  {
    "name": "Isabella Wright",
    "testimonial":
        "Nutritionist's coaching has improved my relationship with food. The emphasis on balance and moderation has made a significant impact on my overall well-being."
  },
  {
    "name": "Mason Turner",
    "testimonial":
        "I appreciate the flexibility in Nutritionist's approach. The coaching is adaptable to my busy schedule, making it easier to maintain a healthy lifestyle. Great guidance!"
  },
  {
    "name": "Zoe Martinez",
    "testimonial":
        "Nutritionist's coaching is informative and practical. I've gained valuable knowledge about nutrition, and the personalized advice has been key to achieving my health goals."
  }
];

Map<String, String> pricingHeader = {
  "title": "Our Pricing",
  "subtitle":
      'We outline our flexible and affordable options to support you on your journey to optimal health and nutrition. We believe that everyone deserves access to personalized nutrition guidance and resources',
};

List<Map<String, dynamic>> pricingList = [
  {
    "title": "Basic Plan",
    "label": "Up to 50% off on Yearly Plan",
    "subtitle":
        "Get started on your health journey with our Basic Plan. It includes personalized nutrition coaching, access to our app, meal planning assistance, and email support.",
    "price": {
      "yearly": "\$349",
      "monthly": "\$49",
    },
  },
  {
    "title": "Premium Plan",
    "label": "Up to 50% off on Yearly Plan",
    "subtitle":
        "Upgrade to our Premium Plan for enhanced features. In addition to the Basic Plan, you'll receive video consultations, priority support, and personalized recipe recommendations.",
    "price": {"yearly": "\$499", "monthly": "\$79"},
  },
  {
    "title": "Ultimate Plan",
    "label": "Up to 50% off on Yearly Plan",
    "subtitle":
        "Experience the full benefits of personalized nutrition coaching with our Ultimate Plan. Enjoy all the features of the Premium Plan, along with 24/7 chat support and exclusive workshops.",
    "price": {"yearly": "\$599", "monthly": "\$99"},
  }
];
